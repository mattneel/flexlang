...
Generic ADTs derive generic impl methods. Any type parameter carried by the ADT's
payload gets the matching bound:

```flex
derive(Eq, Show) type Box<T> = | Box(T)
...
Generic records still cannot derive because records are not generic values yet.
